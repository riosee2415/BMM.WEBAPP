...
import { EVENT_HISTORY_LIST_REQUEST } from "../../../reducers/event";

const EventHistory = ({}) => {
  const { st_loadMyInfoDone, me } = useSelector((state) => state.user);
  const { eventHistoryList } = useSelector((state) => state.event);
  ...
  useEffect(() => {
    dispatch({
      type: EVENT_HISTORY_LIST_REQUEST,
      data: {
        datePick,
      },
    });
    ...
    <AdminLayout>
      ...
      <Wrapper padding={`0px 50px`}>
        <Table
          ...
          rowKey="id"
          columns={col}
          dataSource={eventHistoryList ? eventHistoryList : []}
          size="small"
        />
      </Wrapper>
    </AdminLayout>
...
export default withRouter(EventHistory);
