...
router.post("/list", async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("광고 배너 목록을 불러올 수 없습니다.");
  }
});
...
router.post("/create", isAdminCheck, async (req, res, next) => {
  ...
  const histyoryInsertQuery = `
    INSERT  INTO    advertiseBannerHistory
    (
        value,
        updator,
        createdAt,
  ...
  try {
    ...
    await models.sequelize.query(histyoryInsertQuery);

    return res.status(201).json({ result: true });
  } catch (error) {
    console.error(error);
    return res.status(401).send("광고 배너를 등록할 수 없습니다");
  }
});

/**
 * SUBJECT : 광고 배너 수정
 * PARAMETERS : id, imagePath, link
 * ORDER BY : -
 * STATEMENT : -
 * DEVELOPMENT : 신태섭
 * DEV DATE : 2023/03/02
...
router.post("/update", isAdminCheck, async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("광고 배너를 수정할 수 없습니다.");
  }
});
...
router.post("/delete", isAdminCheck, async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("광고 배너를 삭제할 수 없습니다.");
  }
});
