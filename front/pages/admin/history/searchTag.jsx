...
import { SEARCHTAG_HISTORY_LIST_REQUEST } from "../../../reducers/searchTag";

const SearchTagHistory = ({}) => {
  const { st_loadMyInfoDone, me } = useSelector((state) => state.user);
  const { searchTagHistoryList } = useSelector((state) => state.searchTag);
  ...
  useEffect(() => {
    dispatch({
      type: SEARCHTAG_HISTORY_LIST_REQUEST,
      data: {
        datePick,
      },
    });
    ...
    <AdminLayout>
      ...
      <Wrapper padding={`0px 50px`}>
        <Table
          ...
          rowKey="id"
          columns={col}
          dataSource={searchTagHistoryList ? searchTagHistoryList : []}
          size="small"
        />
      </Wrapper>
    </AdminLayout>
...
export default withRouter(SearchTagHistory);
